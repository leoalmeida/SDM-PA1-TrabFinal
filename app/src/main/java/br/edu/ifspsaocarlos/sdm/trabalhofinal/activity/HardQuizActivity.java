...
public class HardQuizActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    private int quantLetras=0, tamanhoResp=0,tamanhoBase=0;
        ...
        if (quantLetras==tamanhoResp){
            if (respCerta.compareTo(respEscolhida)==0){
                ...
                if (saveGame.getFase()+1 < tamanhoBase){
                    Intent myIntent = new Intent(HardQuizActivity.this, HardQuizActivity.class);
                    saveGame.setFase(saveGame.getFase()+1);
                    saveGameDAO.alterar(saveGame);
                    HardQuizActivity.this.startActivity(myIntent);
                }else{
                    saveGame.setFase(0);
                    saveGameDAO.alterar(saveGame);
                    toast = Toast.makeText(getApplicationContext(), "Você completou o jogo", Toast.LENGTH_SHORT);
                    toast.setGravity(Gravity.CENTER_HORIZONTAL | Gravity.CENTER_VERTICAL, 0, 0);
                    toast.show();
                }
                finish();
